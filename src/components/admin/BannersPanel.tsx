...
import { MAIN_CATEGORY_DEFINITIONS, mainCategorySlugForProduct } from '../../lib/mainCategory';
...
export const BannersPanel: React.FC = () => {
  ...
  const bannerMainCategories=useMemo(()=>{
    const used=new Set(
      products
        .filter((product)=>product.status!=='Draft' && product.product_type!=='bundle')
        .map((product)=>mainCategorySlugForProduct(product,categories))
    );
    return MAIN_CATEGORY_DEFINITIONS.filter((main)=>main.slug!=='other' && used.has(main.slug));
  },[products,categories]);
