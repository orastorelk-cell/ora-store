  ...
  const handleCreateProduct = async (e: React.FormEvent) => {
    ...
    let variants=(productForm.variants || []).map((v,index)=>{
      ...
      return {
        ...
        delivery_price_shift_applied: productAutoPricing ? 0 : v.delivery_price_shift_applied,
    ...
    const finalProductForm = {
      ...savedProductFields,
      delivery_price_shift_applied: productAutoPricing ? 0 : productForm.delivery_price_shift_applied,
            ...
            <form onSubmit={handleCreateProduct} className="space-y-3 text-xs">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div className="sm:col-span-2 rounded-2xl border border-emerald-500/25 bg-emerald-500/5 p-4 space-y-3">
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-black text-emerald-300">O-RA Auto Price Calculator</p><p className="text-[10px] text-neutral-500">Buying Price + O-RA Profit makes the base selling price. Delivery split is added separately to customer display and is never counted as profit.</p></div><div className="flex rounded-xl border border-neutral-700 bg-neutral-950 p-1"><button type="button" onClick={()=>{setProductAutoPricing(true);const profit=profitForBuyingPrice(productForm.buying_price);setProductForm(prev=>({...prev,auto_price_enabled:true,selling_price:Number(prev.buying_price||0)+profit,discount_price:Number(prev.buying_price||0)+profit,discount_enabled:false,offer_buying_price:undefined,supplier_offer_enabled:false}));}} className={`rounded-lg px-3 py-1.5 text-[10px] font-black ${productAutoPricing?'bg-emerald-500 text-black':'text-neutral-400'}`}>AUTO PROFIT</button><button type="button" onClick={()=>{setProductAutoPricing(false);setProductForm(prev=>({...prev,auto_price_enabled:false}));}} className={`rounded-lg px-3 py-1.5 text-[10px] font-black ${!productAutoPricing?'bg-amber-500 text-black':'text-neutral-400'}`}>CUSTOM PROFIT</button></div></div>
                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-4">
                    ...
                    <label className="text-[10px] font-bold text-neutral-400">Base Selling (No Delivery)<input type="number" min="0" required value={productForm.selling_price} onChange={(e)=>{setProductAutoPricing(false);setProductForm(prev=>({...prev,auto_price_enabled:false,selling_price:Math.max(0,Number(e.target.value||0))}));}} className="mt-1 w-full rounded-xl border border-neutral-800 bg-neutral-950 px-3 py-2 text-white"/></label>
                    <div className="rounded-xl border border-emerald-500/20 bg-neutral-950 p-3"><p className="text-[9px] font-bold text-neutral-500">CUSTOMER ITEM PRICE</p><p className="mt-1 text-lg font-black text-emerald-300">Rs. {Math.max(0,Number(productForm.selling_price||0)-Number(productForm.delivery_price_shift_applied||0)+autoDeliveryReserve+(settings.free_delivery_enabled?visibleDeliveryCharge:0)).toLocaleString()}</p><p className="text-[9px] text-neutral-500">{settings.free_delivery_enabled?'FREE delivery shown':`+ Rs. ${visibleDeliveryCharge.toLocaleString()} delivery at order`}</p><p className="mt-1 text-[8px] text-cyan-400">Includes Rs. {autoDeliveryReserve.toLocaleString()} delivery share • not profit</p></div>
                  </div>
                  <div className="grid grid-cols-2 gap-1 text-[9px] text-neutral-500 sm:grid-cols-5"><span>1–249 → +200</span><span>250–499 → +350</span><span>500–999 → +700</span><span>1,000–1,999 → +900</span><span>2,000–2,999 → +1,100</span><span>3,000–4,999 → +1,500</span><span>5,000–7,499 → +1,800</span><span>7,500–9,999 → +2,200</span><span>10,000–14,999 → +3,000</span><span>15,000–19,999 → +4,000</span><span>20,000+ → 25%</span></div>
