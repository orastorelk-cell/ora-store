...
export const productCatalogOfferColumnsPatch = () => ({
  ...
  transform(code: string, rawId: string) {
    ...
    const imports = importMarker
      ...
      + "import { bundleComponentOfferDisplay } from './bundleComponentOfferDisplay';\n";
    ...
    const newSync = `const catalogOfferMetaForSelection = (product: any, variant: any, settings: Record<string, any>) => {\n  const currentPrice = displayUnitPrice(product, settings as any, variant);\n  const regularPrice = regularDisplayUnitPrice(product, settings as any, variant);\n  const savedDiscountPercent = selectionDiscountPercent(product, variant, settings as any);\n\n  // If a real saved/supplier discount is already displayed, mirror that storefront\n  // crossed price. Otherwise calculate only the display-only percentage offer.\n  if (savedDiscountPercent > 0 && regularPrice > currentPrice + 0.001) {\n    return { crossedPrice: regularPrice, actualSellingPrice: currentPrice, offerPercent: savedDiscountPercent };\n  }\n\n  const preview = calculateRoundSpecialOffer({\n    currentPrice,\n    enabled: normalizedProductType(product) !== 'bundle' && roundSpecialOfferEnabledForProduct(product),\n    percent: roundSpecialOfferPercentForSelection(product, variant),\n    hasExistingDiscount: false,\n  });\n  return preview.active\n    ? { crossedPrice: preview.regularPrice, actualSellingPrice: currentPrice, offerPercent: preview.percent }\n    : { crossedPrice: 0, actualSellingPrice: currentPrice, offerPercent: 0 };\n};\n\nconst catalogProductsWithOfferMeta = (products: any[], settings: Record<string, any>) => {\n  const rawProducts = Array.isArray(products) ? products : [];\n  const sourceProducts = catalogProductsForSheet(rawProducts);\n  return sourceProducts.map((product: any) => {\n    const comboMeta = normalizedProductType(product) === 'bundle'\n      ? bundleComponentOfferDisplay(product, rawProducts as any, settings as any)\n      : null;\n    const baseActualSellingPrice = displayUnitPrice(product, settings as any);\n    const productMeta = comboMeta?.active\n      ? { crossedPrice: comboMeta.referencePrice, actualSellingPrice: baseActualSellingPrice, offerPercent: comboMeta.percent }\n      : catalogOfferMetaForSelection(product, undefined, settings);\n    return {\n      ...product,\n      sheet_crossed_price: productMeta.crossedPrice,\n      sheet_actual_selling_price: productMeta.actualSellingPrice,\n      sheet_offer_percent: productMeta.offerPercent,\n      variants: Array.isArray(product?.variants)\n        ? product.variants.map((variant: any) => {\n            const inheritedProductId = String(variant?.sheet_inherited_component_product_id || '');\n            const inheritedVariantId = String(variant?.sheet_inherited_component_variant_id || '');\n            const resolvedCombo = normalizedProductType(product) === 'bundle' && inheritedProductId && inheritedVariantId\n              ? {\n                  ...product,\n                  bundle_components: (product.bundle_components || []).map((component: any) =>\n                    String(component?.product_id || '') === inheritedProductId\n                      ? { ...component, variant_id: inheritedVariantId }\n                      : component\n                  ),\n                }\n              : product;\n            const comboVariantMeta = normalizedProductType(product) === 'bundle'\n              ? bundleComponentOfferDisplay(resolvedCombo as any, rawProducts as any, settings as any)\n              : null;\n            const variantActualSellingPrice = displayUnitPrice(product, settings as any, variant);\n            const variantMeta = comboVariantMeta?.active\n              ? { crossedPrice: comboVariantMeta.referencePrice, actualSellingPrice: variantActualSellingPrice, offerPercent: comboVariantMeta.percent }\n              : catalogOfferMetaForSelection(product, variant, settings);\n            return {\n              ...variant,\n              sheet_crossed_price: variantMeta.crossedPrice,\n              sheet_actual_selling_price: variantMeta.actualSellingPrice,\n              sheet_offer_percent: variantMeta.offerPercent,\n            };\n          })\n        : product?.variants,\n    };\n  });\n};\n\nexport async function syncProductCatalogToGoogleSheets(\n  products: any[],\n  webhookUrl: string,\n  settings: Record<string, any> = {},\n): Promise<SheetActionResult> {\n  const catalogProducts = catalogProductsWithOfferMeta(products, settings);\n  const posted = await postToAppsScript(webhookUrl, { action: 'catalog_sync', products: catalogProducts });\n  if (!posted.ok) return { success: false, message: posted.error || 'Google Sheet catalog sync failed.' };\n  const err = expectStatus(posted.result, ['catalog_synced']);\n  if (err) return { success: false, message: err };\n  return { success: true, message: 'Product catalog synced to Google Sheet.', rows: Number(posted.result?.rows || 0), status: posted.result?.status, version: posted.result?.version };\n}`;

    // Another pre-transform can legitimately adjust this function before this patch runs.
    // Prefer the exact known source, but fall back to replacing the complete exported
    // function block by stable neighbouring export markers instead of failing the build.
    if (text.includes(oldSync)) {
      text = text.replace(oldSync, newSync);
    } else if (!text.includes('const catalogProductsWithOfferMeta =')) {
      const start = text.indexOf('export async function syncProductCatalogToGoogleSheets(');
      const end = start >= 0 ? text.indexOf('\n\nexport async function clearGoogleSheetTestData', start) : -1;
      if (start < 0 || end < 0) {
        throw new Error('[O-RA catalog offer columns] catalog sync metadata marker not found');
      }
      text = text.slice(0, start) + newSync + text.slice(end);
    }
