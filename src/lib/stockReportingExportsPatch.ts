const replaceRequired = (text: string, from: string, to: string, label: string) => {
  ...
  if (!text.includes(from)) throw new Error(`[O-RA stock reports] ${label} marker not found`);
...
export const stockReportingExportsPatch = () => ({
  ...
  transform(code: string, rawId: string) {
    ...
    if (!text.includes("import { jsPDF } from 'jspdf';")) {
      const importMarker = "import { ResponsiveContainer, BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend } from 'recharts';";
    ...
    if (!text.includes('const stockReportAdjustmentNetForSku=')) {
      ...
      const helpers = [
        ...
        '      if(String(p.sku||\'\').trim().toUpperCase()===key)return true;',
        '      return (p.variants||[]).some((v)=>String(v.sku||\'\').trim().toUpperCase()===key);',
        '    });',
        '    if(!product)return 0;',
      ...
      text = replaceRequired(text, marker, helpers, 'stock report adjustment helper');
    }
    ...
    if (!text.includes('const reportAvailable=purchases.length')) {
      ...
      text = replaceRequired(text, from, to, 'signed stock report balance');
    }
    ...
    if (!text.includes('const packedProfitRows = useMemo(')) {
      ...
      const block = [
        ...
        "    doc.setFont('helvetica','normal');doc.setFontSize(9);y+=7;doc.text('Grouped by Item Code | Includes non-cancelled stock-allocated orders (packed / shipped / delivered).',14,y);",
        ...
        '    const drawHeader=()=>{',
        "      doc.setFont('helvetica','bold');doc.setFontSize(8);columns.forEach(([label,x])=>doc.text(label,x,y));",
        '      y+=2;doc.line(14,y,287,y);y+=5;',
        '    };',
        '    drawHeader();',
        ...
        "    y+=4;doc.line(14,y,287,y);y+=6;doc.setFont('helvetica','bold');doc.setFontSize(9);",
      ...
      text = replaceRequired(text, marker, block, 'export helpers');
    }
    ...
    if (!text.includes('Download Stock CSV')) {
      ...
      text = replaceRequired(text, from, to, 'stock header export buttons');
      ...
      text = replaceRequired(text, closeFrom, closeTo, 'stock header button group close');
    }
