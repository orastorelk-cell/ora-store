...
`;
      ...
      const deliveryAwareUpdate = "      updates.set(id,{items:nextItems,subtotal,special_offer_discount,delivery_fee,gift_wrap_selected,gift_wrap_fee,total_amount,is_advance_required:adv,advance_amount:adv?Math.round(total_amount*pct/100):0,call_center_status:'Confirmed',order_status:'Processing',call_center_updated_at:now,stock_allocated:false,stock_status:'Waiting for Stock',...(confirmedAddress?{address:confirmedAddress}:{}),...(confirmedCity?{city:confirmedCity}:{}),...(confirmedDistrict?{district:confirmedDistrict}:{}),...(cityChanged?{fardar_city:undefined,city_verified:false,city_mapping_source:undefined}:{}),product_change_history:changed?[...(order.product_change_history||[]),{changed_at:now,changed_by:'Call Center Confirm Upload',old_items:oldShape,new_items:newShape,reason:reason||undefined}]:(order.product_change_history||[]),notes:[order.notes,cancelled.length?`Call Center cancelled ${cancelled.length} item row(s).`:'',reason?`Call Center: ${reason}`:''].filter(Boolean).join(' | ')});";
      const rebalanceAwareUpdate = "      updates.set(id,{items:nextItems,subtotal,special_offer_discount,delivery_rebalance_offset,delivery_fee,gift_wrap_selected,gift_wrap_fee,total_amount,is_advance_required:adv,advance_amount:adv?Math.round(total_amount*pct/100):0,call_center_status:'Confirmed',order_status:'Processing',call_center_updated_at:now,stock_allocated:false,stock_status:'Waiting for Stock',...(confirmedAddress?{address:confirmedAddress}:{}),...(confirmedCity?{city:confirmedCity}:{}),...(confirmedDistrict?{district:confirmedDistrict}:{}),...(cityChanged?{fardar_city:undefined,city_verified:false,city_mapping_source:undefined}:{}),product_change_history:changed?[...(order.product_change_history||[]),{changed_at:now,changed_by:'Call Center Confirm Upload',old_items:oldShape,new_items:newShape,reason:reason||undefined}]:(order.product_change_history||[]),notes:[order.notes,cancelled.length?`Call Center cancelled ${cancelled.length} item row(s).`:'',reason?`Call Center: ${reason}`:''].filter(Boolean).join(' | ')});";
      const currentSplitAwareUpdate = "      updates.set(id,{items:nextItems,subtotal,special_offer_discount,delivery_rebalance_qty_offer:order.delivery_rebalance_qty_offer,delivery_rebalance_qty_offer_amount,delivery_rebalance_amount_snapshot:order.delivery_rebalance_amount_snapshot,delivery_visible_fee_snapshot:order.delivery_visible_fee_snapshot,delivery_fee,gift_wrap_selected,gift_wrap_fee,total_amount,is_advance_required:adv,advance_amount:adv?Math.round(total_amount*pct/100):0,call_center_status:'Confirmed',order_status:'Processing',call_center_updated_at:now,stock_allocated:false,stock_status:'Waiting for Stock',...(confirmedAddress?{address:confirmedAddress}:{}),...(confirmedCity?{city:confirmedCity}:{}),...(confirmedDistrict?{district:confirmedDistrict}:{}),...(cityChanged?{fardar_city:undefined,city_verified:false,city_mapping_source:undefined}:{}),product_change_history:changed?[...(order.product_change_history||[]),{changed_at:now,changed_by:'Call Center Confirm Upload',old_items:oldShape,new_items:newShape,reason:reason||undefined}]:(order.product_change_history||[]),notes:[order.notes,cancelled.length?`Call Center cancelled ${cancelled.length} item row(s).`:'',reason?`Call Center: ${reason}`:''].filter(Boolean).join(' | ')});";
      const qtyOfferAwareUpdate = "      updates.set(id,{items:nextItems,subtotal,special_offer_discount,delivery_rebalance_qty_offer:order.delivery_rebalance_qty_offer,delivery_fee,gift_wrap_selected,gift_wrap_fee,total_amount,is_advance_required:adv,advance_amount:adv?Math.round(total_amount*pct/100):0,call_center_status:'Confirmed',order_status:'Processing',call_center_updated_at:now,stock_allocated:false,stock_status:'Waiting for Stock',...(confirmedAddress?{address:confirmedAddress}:{}),...(confirmedCity?{city:confirmedCity}:{}),...(confirmedDistrict?{district:confirmedDistrict}:{}),...(cityChanged?{fardar_city:undefined,city_verified:false,city_mapping_source:undefined}:{}),product_change_history:changed?[...(order.product_change_history||[]),{changed_at:now,changed_by:'Call Center Confirm Upload',old_items:oldShape,new_items:newShape,reason:reason||undefined}]:(order.product_change_history||[]),notes:[order.notes,cancelled.length?`Call Center cancelled ${cancelled.length} item row(s).`:'',reason?`Call Center: ${reason}`:''].filter(Boolean).join(' | ')});";
      const newUpdate = "      updates.set(id,{items:nextItems,subtotal,special_offer_discount:stableQtyOfferDiscount,delivery_rebalance_qty_offer:order.delivery_rebalance_qty_offer,delivery_rebalance_qty_offer_amount:stableDeliveryQtyOffer,delivery_rebalance_amount_snapshot:order.delivery_rebalance_amount_snapshot,delivery_visible_fee_snapshot:order.delivery_visible_fee_snapshot,delivery_fee,gift_wrap_selected,gift_wrap_fee,total_amount:stableTotalAmount,is_advance_required:adv,advance_amount:adv?Math.round(stableTotalAmount*pct/100):0,...(confirmedAddress?{address:confirmedAddress}:{}),city:sheetCity,district:sheetDistrict,...(cityChanged?{fardar_city:undefined,city_verified:false,city_mapping_source:undefined}:{}),confirm_upload_batch_id:uploadPackingBatchId,invoice_confirm_snapshot:invoiceConfirmSnapshot,call_center_status:'Confirmed',order_status:'Processing',call_center_updated_at:now,stock_allocated:false,stock_status:'Waiting for Stock',product_change_history:changed?[...(order.product_change_history||[]),{changed_at:now,changed_by:'Call Center Confirm Upload',old_items:oldShape,new_items:newShape,reason:reason||undefined}]:(order.product_change_history||[]),notes:[order.notes,cancelled.length?`Call Center cancelled ${cancelled.length} item row(s).`:'',reason?`Call Center: ${reason}`:''].filter(Boolean).join(' | ')});";
      ...
      else if (text.includes(deliveryAwareUpdate)) text = text.replace(deliveryAwareUpdate, newUpdate);
      else if (text.includes(rebalanceAwareUpdate)) text = text.replace(rebalanceAwareUpdate, newUpdate);
      else if (text.includes(qtyOfferAwareUpdate)) text = text.replace(qtyOfferAwareUpdate, newUpdate);
      else if (text.includes(currentSplitAwareUpdate)) text = text.replace(currentSplitAwareUpdate, newUpdate);
