...
/**
 * Repair packing output is intentionally separated:
 * - A4 4-Up contains SINGLE-PAGE orders only.
 * - Multi-page orders get their own repaired PDF so page 1/2, 2/2, etc. stay together
 *   and never get mixed with unrelated single-page packing invoices.
  ...
  transform(code: string, rawId: string) {
    ...
    if (id.endsWith('/src/lib/pdfGenerator.ts')) {
      if (!text.includes('export async function generateRepairedA4FourUpInvoicesPDFFromCsv')) {
        ...
        const addition = marker + `\nexport async function generateRepairedA4FourUpInvoicesPDFFromCsv(orders:Order[], settings:StoreSettings = {} as StoreSettings, csvText='', fileName?:string) {\n  if(!orders.length) throw new Error('No invoices are available in this packing batch.');\n  const repaired:Order[]=[];\n  for(const order of orders){\n    const snapshot=repairSnapshotFromCsv(order,csvText);\n    repaired.push(await buildRepairedInvoiceOrder({...(order as any),invoice_confirm_snapshot:snapshot} as Order,settings));\n  }\n  const singlePage=repaired.filter(order=>getInvoicePageCount(order)===1);\n  if(!singlePage.length) throw new Error('This repaired batch has no single-page invoices for A4 4-up printing. Use Repair Multi-page from CSV for multi-page orders.');\n  await generateA4FourUpInvoicesPDF(singlePage,settings,fileName || ('O-RA_REPAIRED_A4_4-UP_'+Date.now()+'.pdf'));\n}\n\nexport async function generateRepairedMultiPageInvoicesPDFFromCsv(orders:Order[], settings:StoreSettings = {} as StoreSettings, csvText='', fileName?:string) {\n  if(!orders.length) throw new Error('No multi-page invoices are available in this packing batch.');\n  const repaired:Order[]=[];\n  for(const order of orders){\n    const snapshot=repairSnapshotFromCsv(order,csvText);\n    repaired.push(await buildRepairedInvoiceOrder({...(order as any),invoice_confirm_snapshot:snapshot} as Order,settings));\n  }\n  const multiPage=repaired.filter(order=>getInvoicePageCount(order)>1);\n  if(!multiPage.length) throw new Error('This repaired batch has no multi-page invoices.');\n  await generateBatchInvoicesPDF(multiPage,settings,fileName || ('O-RA_REPAIRED_MULTI-PAGE_'+Date.now()+'.pdf'));\n}\n`;
