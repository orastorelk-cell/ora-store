...
export const specialOfferPercentageRulePatch = () => ({
  ...
  transform(code: string, rawId: string) {
    ...
    if (id.endsWith('/src/components/admin/AdminDashboard.tsx')) {
      ...
      // Put the ON/OFF + main percentage beside the O-RA Price Calculator.
      // Use the profit-table row as the stable insertion point so wording changes
      // in Customer Display / delivery reserve help text do not break production builds.
      const profitTableMarker = `                  <div className="grid grid-cols-2 gap-1 text-[9px] text-neutral-500 sm:grid-cols-5">`;
      const offerControl = `                  {(() => {
...
`;
      if (!text.includes('SPECIAL OFFER DISPLAY')) {
        text = replaceRequired(text, profitTableMarker, offerControl + profitTableMarker, 'price calculator special offer control');
      }
