...
orderTime:String(order?.created_at||new Date().toISOString()),
leadId:String(order?.platform_lead_id||''),
importedStatus:String(order?.call_center_status||'Pending'),
offer:orderQtyOfferLabelServer(order,settings),
discount:orderSheetCombinedDiscountServer(order),
...
qtyOfferRules:sheetQtyOfferRulesServer(settings,order),
items:(Array.isArray(order?.items)?order.items:[]).map((it:any)=>({
mainCode:String(it?.main_sku||it?.sku||''),
variant:String(it?.variant_name||''),
itemCode:String(it?.sku||''),
itemName:String(it?.product_name||''),
qty:Math.max(1,Number(it?.quantity||1)),
unitPrice:Number(it?.unit_price||0),
lineTotal:Math.max(1,Number(it?.quantity||1))*Number(it?.unit_price||0),
})),
});
...
app.delete('/api/orders', requireSuperAdmin, async (_req,res)=>{
  ...
  try {
    ...
    if (sheetWebhookUrl) {
      postAppsScriptFastServer(sheetWebhookUrl, { payload_type: 'clear_orders' }).catch(() => {});
    }
  } catch {}
  return res.json({ok:true});
});
...
async function startServer() {
  if (process.env.NODE_ENV !== "production") {
    const vitePkg = 'vite';     const { createServer: createViteServer } = await import(vitePkg);
