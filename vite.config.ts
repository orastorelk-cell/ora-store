...
import { purchaseHistoryCorrectionPatch } from './src/lib/purchaseHistoryCorrectionPatch';
import { purchaseHistoryStockDurabilityPatch } from './src/lib/purchaseHistoryStockDurabilityPatch';
import { waybillAutoAssignmentServerGuardPatch } from './src/lib/waybillAutoAssignmentServerGuardPatch';
import { stockReportingExportsPatch } from './src/lib/stockReportingExportsPatch';
import { fardarDeliveredProfitReportPatch } from './src/lib/fardarDeliveredProfitReportPatch';
import { deliveredCsvUploadPatch } from './src/lib/deliveredCsvUploadPatch';
...
import { confirmUploadIgnoredDetailsPatch } from './src/lib/confirmUploadIgnoredDetailsPatch';
import { confirmUploadResultPersistencePatch } from './src/lib/confirmUploadResultPersistencePatch';
import { confirmUploadServerOrderLookupPatch } from './src/lib/confirmUploadServerOrderLookupPatch';
...
import { facebookComboVariantLeadPatch } from './src/lib/facebookComboVariantLeadPatch';
...
import { adminOrdersConfirmedFilterPatch } from './src/lib/adminOrdersConfirmedFilterPatch';
...
import { productCardComboVariantGuardPatch } from './src/lib/productCardComboVariantGuardPatch';
...
import { comboPackItemSearchPatch } from './src/lib/comboPackItemSearchPatch';
import { comboPackManualContentPatch } from './src/lib/comboPackManualContentPatch';
import { customerComboVariantSelectorPatch } from './src/lib/customerComboVariantSelectorPatch';
...
import { waybillAssignmentAtomicPatch } from './src/lib/waybillAssignmentAtomicPatch';
...
import { tiktokProfileSettingsPatch } from './src/lib/tiktokProfileSettingsPatch';
import { confirmedPurchaseSocialProofPatch } from './src/lib/confirmedPurchaseSocialProofPatch';

export default defineConfig(() => {
  return {
    plugins: [leadItemCodeBlankDefaultPatch(), waybillExistingGarbageCleanupPatch(), waybillDuplicateSafetyPatch(), deliveryCityAcceptancePatch(), maintenanceBilingualNoticePatch(), mainCategoryHierarchyPatch(), wishlistRoundOfferCompatPatch(), roundSpecialOfferPatch(), specialOfferPercentageRulePatch(), specialOfferAdminPreviewPatch(), specialOfferLivePreviewDecimalPatch(), roundSpecialOfferPercentBadgePatch(), productCardOfferLayoutPatch(), productCardComboVariantGuardPatch(), bundleComponentOfferDisplayPatch(), qtyOfferMergedDisplayPatch(), checkoutCombinedOfferPatch(), bundleCheckoutOfferPatch(), productCatalogOfferColumnsPatch(), googleAppsScriptCurrencyParsePatch(), googleAppsScriptOrderCrossPricePatch(), orderCrossPriceActualSourcePatch(), productBackupDryRunPatch(), welcomeSplashAppPatch(), customerCartWordingPatch(), comboPackItemSearchPatch(), comboPackManualContentPatch(), customerComboVariantSelectorPatch(), adminDashboardStoragePatch(), adminDashboardConfirmedFinancePatch(), adminDashboardOutOfStockVariantPatch(), adminOrdersConfirmedFilterPatch(), freeDeliveryQtyOfferGuardPatch(), adminDashboardUnifiedUploadPatch(), confirmUploadPackingBatchPatch(), waybillAssignmentAtomicPatch(), restockPackingPurchaseSessionPatch(), restockPurchaseAtomicSafetyPatch(), waitingRestockBatchMarkerPatch(), purchaseHistoryCorrectionPatch(), purchaseHistoryStockDurabilityPatch(), waybillAutoAssignmentServerGuardPatch(), stockReportingExportsPatch(), fardarDeliveredProfitReportPatch(), deliveredCsvUploadPatch(), fullPaymentConfirmUploadPatch(), confirmUploadCrossPriceSnapshotPatch(), confirmUploadCustomerNamePatch(), confirmUploadIgnoredDetailsPatch(), confirmUploadServerOrderLookupPatch(), confirmUploadResultPersistencePatch(), packingDownloadUxPatch(), pdfPerformancePatch(), invoiceRepairMoneyParsingPatch(), invoiceCrossedPriceMathPatch(), invoiceCombinedDiscountPatch(), invoiceSnapshotConsistencyPatch(), repairA4FourUpCsvPatch(), a5InvoiceDownloadPatch(), packingBatchCombinePatch(), adminDashboardFardarHistoryPatch(), adminDashboardFardarHistoryDurablePatch(), adminDashboardVisibleTemplatePatch(), adminDashboardLeadPreviewPatch(), adminDashboardLeadServerPreviewPatch(), leadMetaCsvCompatibilityPatch(), facebookComboVariantLeadPatch(), adminDashboardProductAutoPopularPatch(), productEnglishOnlyPatch(), productSharePatch(), sheetVariantScopePatch(), invoiceDesignLiveCatalogPatch(), adminProductListDeliveryDisplayPatch(), facebookPageSettingsPatch(), tiktokProfileSettingsPatch(), confirmedPurchaseSocialProofPatch(), react(), tailwindcss()],
    resolve: {
      alias: {
